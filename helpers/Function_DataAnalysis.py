...
def Slicing(data, t_start=None, t_end=None):
    ...
    for label in data.keys():
        ...
        newname = label+'r'
        if nrCell > 1000:
            dset = data.create_dataset(newname,data=data[label][int(t_start/0.025):t_end])
            #data[label][...] = data[label][int(t_start/0.025):t_end]
        else:
            dset = data.create_dataset(newname, data=[data[label][k][t_start:t_end] for k in range(0,nrCell)])
            #data[label][...] = [data[label][k][t_start:t_end] for k in range(0,nrCell)]
        #del data[label]
...
def SlicingSpikes(data, t_start=float("-inf"), t_end=float("inf")):
    ...
    for label in data.keys():
        ...
        for arr in collection:
            ...
            #before 'not d=, just output..' 
        newname = label+'r'
        newset = data.create_dataset(newname,data=output[label],dtype=dt_type)
            #del data[label]
     ...
     #   output = []
      #  for j in data:
       #     x = np.array(j)
        ...
        #newset = data.create_dataset('testDCN',data=output,dtype=dt_type)
